...
function percentBottomPadding(compact) {
  const badgeH = labelBadgeHeight(compact);
  return badgeH * 2 + monthLabelBandHeight(compact) + 28;
}
...
function barBaseLineY(barPrimary, barSecondary) {
  return Math.max(barPrimary.base, barSecondary.base);
}
...
function drawPercentComboLabels(chart, opts) {
  ...
  for (let i = 0; i < panelData.labels.length; i += 1) {
    ...
    const barBase = barBaseLineY(barPrimary, barSecondary);
    ...
    const dimPrimaryTop = barBase + gap;
    const dimSecondaryTop = dimPrimaryTop + badgeH + 4;
      ...
      scales: {
        x: {
          ticks: {
            ...
            color: "#747678",
            maxRotation: 0,
            padding: labelsOn && percentMode ? (compact ? 14 : 20) : compact ? 8 : 16,
          },
          grid: { display: false },
        },
        yCount: {
